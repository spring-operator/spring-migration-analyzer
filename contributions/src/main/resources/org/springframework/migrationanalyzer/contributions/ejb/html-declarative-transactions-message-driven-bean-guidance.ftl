The application contains a number of message-driven beans (MDBs) that use declarative JTA transactions. It should be noted that the default MDB configuration
...
<p />
If the transaction does not involve any resources, i.e. the transaction was not actually necessary, the bean can be easily migrated to a Spring message-driven POJO
with no declarative transaction configuration.
<p />
If the transaction only involves a single resource, it can be easily migrated to a Spring message-driven POJO that uses Spring's support for
